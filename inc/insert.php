...
if(isset($_POST)){
	...
	if(isset($_GET['add_topic'])){
		...
		$sql_select_max_reihenf = "SELECT MAX(REIHENF) AS MAX FROM (SELECT REIHENF FROM ".DB_PREFIX."topics WHERE ID_EPISODE = '".$_SESSION['cur_episode']."' AND ID_PODCAST = '".$_SESSION['podcast']."' AND ID_CATEGORY ='".$category."' UNION ALL SELECT REIHENF FROM ".DB_PREFIX."links WHERE ID_EPISODE = '".$_SESSION['cur_episode']."' AND ID_PODCAST = '".$_SESSION['podcast']."' AND ID_CATEGORY ='".$category."') AS QUERY";
